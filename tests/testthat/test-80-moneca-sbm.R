...
test_that("mat.list length matches segment.list and mat.list[[1]] has margins", {
  ...
  # Margin preserved: last row/col equals sum of core
  mx1 <- out$mat.list[[1]]
  n <- nrow(mx1)
...
test_that("segment.membership.dataframe works on moneca_sbm output", {
  skip_if_no_greed()
  mob <- get_test_data(size = "medium", seed = 42)
  out <- moneca_sbm(mob, backend = "greed", seed = 1L)
  df <- segment.membership.dataframe(out)
  expect_s3_class(df, "data.frame")
  expect_true(nrow(df) >= (nrow(mob) - 1L))
})

test_that("segment.quality works on moneca_sbm output", {
  skip_if_no_greed()
  mob <- get_test_data(size = "medium", seed = 42)
  out <- moneca_sbm(mob, backend = "greed", seed = 1L)
  q <- segment.quality(out)
  expect_true(is.list(q) || is.data.frame(q))
})

test_that("print.moneca runs on moneca_sbm output", {
  skip_if_no_greed()
  mob <- get_test_data(size = "medium", seed = 42)
  out <- moneca_sbm(mob, backend = "greed", seed = 1L)
...
test_that("segment.levels truncates backend hierarchy", {
  ...
  # Trimmed has at most segment.levels + 1 levels (atomic + 2)
...
test_that("invalid backend triggers match.arg error", {
  # Localized match.arg messages ("should be one of" vs "dovrebbe essere uno di"):
  # test only that an error is raised, not the specific text.
  expect_error(
    moneca_sbm(get_test_data("small"), backend = "nope")
  )
})
