# Tests: empirical parity between moneca_sbm() and moneca_fast()
# ==============================================================
#
...
# recover segments similar to moneca_fast() on a well-structured mobility
# matrix. This test records the empirical agreement but does NOT gate
# acceptance on a strict threshold — the parity is an empirical question
# whose answer is itself a scientific finding reported in the vignette.
...
# Adjusted Rand Index — compact pure-R implementation to avoid
...
# Normalized Mutual Information (NMI, sqrt variant)
nmi <- function(a, b) {
  tab <- table(a, b)
  n <- sum(tab)
  p_ab <- tab / n
...
# Extract per-node block label at a given level of a moneca object
level_labels <- function(mon, level) {
  ...
  # Unassigned nodes get unique singleton labels
  na_idx <- is.na(labels)
  if (any(na_idx)) {
    labels[na_idx] <- max(0L, labels, na.rm = TRUE) +
      seq_len(sum(na_idx))
  }
  as.integer(labels)
}
...
test_that("moneca_sbm vs moneca_fast: best-match ARI and NMI on a synthetic fixture", {
  skip_if_no_greed()
  ...
  # Compute ARI / NMI across all pairs of levels (fast × sbm, both excluding
  # the atomic level 1) and take the maximum.
  ...
  # Record for visibility in test output
  message(
    sprintf(
      "moneca_sbm vs moneca_fast parity: best-match ARI = %.3f, NMI = %.3f ",
      best$ari,
      best$nmi
    ),
    sprintf(
      "(fast level %d vs sbm level %d).",
      best$fast_lvl,
      best$sbm_lvl
    )
  )

  # Soft, non-strict assertion: agreement should be better than random
  # (ARI > 0) on a well-structured synthetic fixture. The roadmap's
  # aspirational target of ARI >= 0.55 is explored in the SBM vignette,
  # not gated here.
