...
gg.moneca <- function(
  ...
  .Deprecated(
    "plot_moneca_ggraph",
    package = "moneca",
    msg = "gg.moneca() is deprecated. Use plot_moneca_ggraph() instead."
  )
  ...
  if (identical(edge.color, "weight")) {
    ...
    scale_modifications$edge.color <- scale_color_continuous(
      high = "darkblue",
      low = "azure1"
    )
  }
  ...
  p <- eliter::graph.plot(
    ...
    midpoints = midpoints,
    midpoint.arrow = midpoint.arrow,
    edge.text = edge.text,
    edge.text.size = edge.text.size,
    edge.text.alpha = edge.text.alpha
  )
  ...
  segment.circles.hull <- function(layout, group, diameter) {
    ...
    cbind(
      circle.hull,
      group = runif(1, min = 0, max = 999999999),
      membership = membership.seg
    )
  }

  annotate_segments <- function(layout, seg.list, diameter, border.alpha) {
    segment.circles <- lapply(
      seg.list,
      segment.circles.hull,
      layout = layout,
      diameter = max(layout[, 1:2]) / diameter
    )
  ...
  if (identical(show.borders, TRUE)) {
    list.annotate <- lapply(
      seg$segment.list,
      annotate_segments,
      layout = layout,
      diameter = (1 / border.padding) * 20,
  ...
  if (identical(border.text, TRUE) & length(level) > 0) {
    ...
    for (i in 1:length(seg$segment.list)) {
      segment.circles <- lapply(
        ...
        segment.circles.hull,
        layout = layout,
        diameter = border.padding.diameter
      )
      segment.circles <- do.call(rbind, segment.circles)
      seg.circles[[i]] <- segment.circles
    }
    segment.circles <- do.call(rbind, seg.circles)
    max.circles <- aggregate(
      segment.circles$y,
      by = list(segment.circles$membership),
      FUN = max
    )
    max.segments <- segment.circles[
      (segment.circles$y %in% max.circles$x) &
        (segment.circles$membership %in% max.circles$Group.1),
    ]

    max.segments$xend <- max.segments$x +
      ((border.padding.diameter * 2) * (border.text.size / 3.9))
    ...
    list.annotate <- list(
      annotate(
        ...
        x = max.segments$xend,
        y = max.segments$y,
        label = max.segments$membership,
        color = border.text.color,
        size = border.text.size,
        vjust = border.text.vjust,
        hjust = border.text.hjust
      ),
      annotate(
        geom = "segment",
        x = max.segments$x,
    ...
    if (any(tab.mem == 1)) {
      singles.layout <- layout[
        layout$Membership %in% names(tab.mem)[tab.mem == 1],
      ]
      singles.layout <- data.frame(
        x = singles.layout$X,
        y = singles.layout$Y,
        group = runif(1, min = 0, max = 999999999),
        membership = singles.layout$Membership
      )
      singles.layout$y <- singles.layout$y + (border.padding.diameter * 0.25)
      singles.layout$xend <- singles.layout$x +
        ((border.padding.diameter * 2) * (border.text.size / 3.9))
      ...
      list.annotate <- list(
        annotate(
          geom = "text",
          x = singles.layout$xend,
          y = singles.layout$y,
          label = singles.layout$membership,
          color = border.text.color,
          size = border.text.size,
          vjust = border.text.vjust,
          hjust = border.text.hjust
        ...
        annotate(
          geom = "segment",
          x = singles.layout$x,
          xend = singles.layout$xend,
          y = singles.layout$y,
          yend = singles.layout$y,
          color = border.color,
          alpha = border.alpha
        )
      )
...
#'
#' \strong{Deprecated}. Use \code{\link{plot_ego_ggraph}} instead.
#' This function will be removed in moneca 1.2.0.
#'
...
ego.plot <- function(
  ...
  .Deprecated(
    "plot_ego_ggraph",
    package = "moneca",
    msg = "ego.plot() is deprecated. Use plot_ego_ggraph() instead."
  )
  ...
  scales <- list()
  scales$fill <- scale_fill_gradientn(
    colours = brewer.pal(5, color.scheme),
    na.value = "black",
    labels = percent,
    name = "Mobile"
  )
  ...
  if (identical(edge.weight, "discrete")) {
    ...
    scales$color <- scale_color_manual(
      values = brewer.pal(nlevels(edge.weight) + 1, color.scheme)[-1],
      name = "Relativ risiko"
    )
  } else {
    edge.weight <- "weight"
...
#'
#' \strong{Deprecated}. Use \code{\link{plot_stair_ggraph}} instead.
#' This function will be removed in moneca 1.2.0.
#'
...
stair.plot <- function(
  ...
  .Deprecated(
    "plot_stair_ggraph",
    package = "moneca",
    msg = "stair.plot() is deprecated. Use plot_stair_ggraph() instead."
  )
  ...
  list.scales$alpha <- scale_alpha_continuous(
    guide = "none",
    range = c(0.05, 0.9)
  )
  ...
  for (i in seq_along(level.list)[-1]) {
    transition[[i]] <- memberships.list[[i - 1]] == memberships.list[[i]]
  }
