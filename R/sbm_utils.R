...
# ===========================================
#
# Adapter that turns backend-agnostic SBM output into a moneca-class S3 object,
# plus margin / symmetry / diagonal preprocessing shared across backends.
#
# All helpers in this file are internal (@keywords internal / @noRd).
...
.ensure_margins_sbm <- function(mx, has_margins = "auto") {
  ...
  if (nrow(mx) != ncol(mx)) {
    stop(
      ...
      "person-employer input is deferred to moneca_bipartite(); see ",
      "SCALING_ROADMAP.md direction D1).",
      call. = FALSE
    )
  }
...
#
# Input: integer vector of length n_core, values = block IDs.
# Output: list of integer vectors (one per non-singleton block), each giving the
# node indices in that block. Singletons are pruned, mirroring the convention in
# moneca_fast's create.segments.fast() (R/moneca_fast.R:827).
...
#
# Mirrors segment.matrix.fast() defined inside moneca_fast (R/moneca_fast.R:698)
# but exposed at the package level so the SBM adapter can reuse it without
...
#
# SBM backends return finest-to-coarsest levels. MONECA convention: level 2 is
# the finest clustering, level L+1 is the coarsest. If the user passes
# segment.levels = L, we pick L approximately log-spaced levels from the
# backend's hierarchy, preserving the finest and coarsest.

.select_sbm_levels <- function(
  available_K,
  segment.levels = NULL
) {
  ...
  # Approximately log-spaced indices preserving endpoints
...
.sbm_fit_to_moneca <- function(
  ...
  # Level selection from backend's hierarchy
  ...
  # segment.list: level 1 atomic; levels 2..(L+1) from SBM
  ...
  # mat.list: level 1 = mx_full (with margins); subsequent = aggregated
