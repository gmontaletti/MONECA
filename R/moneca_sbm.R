...
#'
#' Alternative to \code{\link{moneca_fast}} that replaces clique enumeration
#' with hierarchical degree-corrected stochastic block model (DC-SBM)
#' inference. MONECA's weight matrix \code{RR = O / E} is the expected-value
#' residual under a DC-SBM with identity block interaction, so DC-SBM
#' clustering is a principled scalable replacement for the clique step.
#'
...
#'   \code{\link{moneca_fast}} parameter.
...
#'   \code{$isolates_summary} slot mirroring \code{\link{moneca_fast}}.
