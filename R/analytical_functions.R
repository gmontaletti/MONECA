...
find.segments <- function(
  ...
  clique.test <- function(potential.clique) {
    if (length(potential.clique) < 2) {
      return(TRUE)
    } # Single nodes are trivially cliques
  ...
  for (i in 1:loop.length) {
    ...
    # Use pre-sorted edge (replaces repeated max() search)
...
moneca_fast <- function(
  ...
  if (segment.levels > 1) {
    ...
    for (i in 2:segment.levels) {
      ...
      if (i > 5 && object.size(out.put) > 100 * 1024 * 1024) {
        # 100MB threshold
        warning(
          "Memory usage is high. Stopping early to prevent system instability."
        )
        break
      }
    }
...
weight.matrix <- function(
  ...
  if (auto_tune) {
    if (tune_verbose) {
      message(
        "Auto-tuning small.cell.reduction parameter using method: ",
        tune_method
      )
    }
...
segment.colors <- function(segments) {
  ...
  for (i in 1:n.segments) {
    colors[[i]] <- segment.grey(segments, i)
  }

  groups <- unlist(lapply(segments$segment.list, length))

  for (i in 1:length(groups)) {
    colors[[i]] <- colors[[i]][1:groups[i]]
  }
...
#' Layout matrix
#'
#' A matrix with the coordinates of the segments using Fruchterman-Reingold algorithm.
#' @param segments A segment object from moneca().
#' @param attraction The distance between the segment points for each level.
#' @param level The included levels.
#' @param mode The mode (directed or undirected).
#' @param weight.adjustment Weight adjustment factor.
#' @param start.temp Starting temperature for the algorithm.
#' @param niter Number of iterations.
#' @param tie.adjustment Tie adjustment factor.
#' @param ... Additional arguments passed to layout algorithm.
#' @return A matrix of x,y coordinates for each node.
#' @export
layout.matrix <- function(
  segments,
  ...
  if (is.null(mx_input)) {
    stop(
      "segments$mat.list[[1]] is NULL - the moneca object appears to be incomplete or corrupted. Please re-run the moneca() function to generate a valid segments object."
    )
  }
  ...
  gra.lay <- moneca_graph_from_adjacency(
    mx.attract,
    mode = "directed",
    weighted = TRUE
  )
  ...
  assign.attraction <- function(mx.attract, segment, attract) {
    for (i in 1:length(segment)) {
      mx.attract[segment[[i]], segment[[i]]] <- attract
    }
    return(mx.attract)
  }
  ...
  layout <- layout_with_fr(
    gra.lay,
    weights = E(gra.lay)$weight * weight.adjustment,
    niter = niter,
...
moneca.plot <- function(
  ...
  .Deprecated(
    "plot_moneca_ggraph",
    package = "moneca",
    msg = "moneca.plot() is deprecated. Use plot_moneca_ggraph() instead."
  )
  ...
  mat.edges <- edges
  gra.edges <- moneca_graph_from_adjacency(
    mat.edges,
    mode = mode,
    weighted = TRUE,
    diag = FALSE
  ...
  plot(
    ...
    mark.groups = segments,
    mark.border = border.col,
    mark.col = NULL,
    mark.expand = 10,
    edge.width = 1,
...
segment.membership.enhanced <- function(
  segments,
  level = seq(segments$segment.list),
  ...
  invalid_membership_indices <- which(
    basic_membership$membership == "" |
      basic_membership$membership == "FALSE" |
      is.na(basic_membership$membership)
  )
  if (length(invalid_membership_indices) > 0) {
    level_names[invalid_membership_indices] <- basic_membership$name[
      invalid_membership_indices
    ]
  }
...
generate_level_name <- function(
  ...
  if (naming_strategy == "pattern" || naming_strategy == "auto") {
    pattern_name <- apply_enhanced_pattern_rules(
      node_names,
      pattern_rules,
      membership_id
    )
...
apply_enhanced_pattern_rules <- function(
  node_names,
  pattern_rules,
  membership_id
) {
  ...
  for (rule_name in names(pattern_rules)) {
    ...
    if (total_score > 0) {
      pattern_scores[[rule_name]] <- list(
        ...
        coverage = sum(sapply(rule$patterns, function(p) {
          sum(grepl(p, node_names, ignore.case = TRUE))
        })) /
          length(node_names)
      )
    }
  ...
  if (length(pattern_scores) > 0) {
    # Sort by score
    sorted_scores <- pattern_scores[order(
      sapply(pattern_scores, function(x) x$score),
      decreasing = TRUE
    )]
    ...
    if (best_match$coverage >= 0.3) {
      ...
      if (length(sorted_scores) >= 2) {
        ...
        if (
          second_best$score > best_match$score * 0.5 &&
          ...
          if (
            any(grepl("Executive|Leadership", categories)) &&
              any(grepl("Management", categories))
          ) {
            return("Executive_Management")
          } else if (
            any(grepl("Professional", categories)) &&
              any(grepl("Technical", categories))
          ) {
...
extract_meaningful_terms <- function(node_names) {
  ...
  frequent_words <- names(word_freq)[
    word_freq >= max(1, length(node_names) * 0.3)
  ]

  if (length(frequent_words) > 0) {
    # Prioritize longer, more specific terms
    best_words <- frequent_words[order(
      nchar(frequent_words),
      decreasing = TRUE
    )]
...
#' Force Segments
#'
#' Creates a two-level segment object with a forced solution based on an external
#' variable. This is an internal function for creating custom segmentation structures.
#'
#' @param segments A moneca object returned by \code{\link{moneca}}.
#' @param variable A vector specifying forced segment assignments.
#'
#' @return A moneca object with forced segmentation at level 2.
#' @keywords internal
...
#'
#' @keywords internal
find_best_level_with_structure <- function(
  segments,
  enhanced_membership,
  max_level
) {
...
extract_named_matrices <- function(segments, max_level, enhanced_membership) {
  ...
  max_level_membership <- enhanced_membership[
    enhanced_membership$level == max_level,
  ]

  # Create mapping of segment numbers to enhanced names
  segment_mapping <- unique(max_level_membership[, c(
    "membership",
    "level_name"
  )])
  ...
  if (nrow(max_level_matrix) - 1 == nrow(segment_mapping)) {
    ...
    new_names <- c(
      segment_mapping$level_name[order(segment_mapping$membership)],
      "Total"
    )
...
compute_centrality_metrics <- function(segments, level, enhanced_membership) {
  ...
  valid_membership <- level_membership[
    level_membership$membership != "FALSE" &
  ...
  valid_membership$membership_clean <- as.numeric(as.character(
    valid_membership$membership
  ))
  segment_names <- unique(valid_membership[, c(
    "membership_clean",
    "level_name"
  )])
  ...
  segment_composition <- sapply(1:nrow(segment_names), function(i) {
    members <- valid_membership[
      valid_membership$membership_clean == segment_names$membership_clean[i],
      "name"
    ]
  ...
  centrality_df <- centrality_df[
    order(centrality_df$strength_total, decreasing = TRUE),
  ]
...
compute_mobility_metrics <- function(segments, level, enhanced_membership) {
  ...
  for (i in 1:n_segments) {
    ...
    mobility_metrics$net_flow[i] <- mobility_metrics$total_inflow[i] -
      mobility_metrics$total_outflow[i]

    # Mobility rates
    row_total <- mat[i, ncol(mat)]
    mobility_metrics$immobility_rate[i] <- if (row_total > 0) {
      mat[i, i] / row_total
    } else {
      0
    }
  ...
  total_mobility <- sum(mat[-nrow(mat), -ncol(mat)]) -
    sum(diag(mat[-nrow(mat), -ncol(mat)]))
  mobility_metrics$relative_mobility_share <-
    (mobility_metrics$total_outflow + mobility_metrics$total_inflow) /
    total_mobility
...
generate_summary_report <- function(
  ...
  consolidation_ratio <- results$metrics_comparison$n_nodes[1] /
    results$metrics_comparison$n_nodes[max_level]
  report_rows[[length(report_rows) + 1]] <- data.frame(
    category = "Network Structure",
    metric = "Consolidation Ratio",
    value = paste0(round(consolidation_ratio, 2), ":1"),
    details = paste(
      results$metrics_comparison$n_nodes[1],
      "->",
      results$metrics_comparison$n_nodes[max_level]
    ),
    stringsAsFactors = FALSE
  ...
  analysis_metrics <- results$metrics_comparison[
    results$metrics_comparison$level == analysis_level,
  ]
  ...
  if (nrow(results$centrality_report) > 0) {
    ...
    for (i in 1:top_n) {
      report_rows[[length(report_rows) + 1]] <- data.frame(
        ...
        details = paste(
          "Strength:",
          round(results$centrality_report$strength_total[i], 2)
        ),
        stringsAsFactors = FALSE
      )
    }
  ...
  if (nrow(results$mobility_report) > 0) {
    ...
    if (length(highest_mobility_idx) > 0) {
      report_rows[[length(report_rows) + 1]] <- data.frame(
        ...
        details = paste(
          "Rate:",
          round(results$mobility_report$mobility_rate[highest_mobility_idx], 3)
        ),
        stringsAsFactors = FALSE
      )
    }
    ...
    if (length(lowest_mobility_idx) > 0) {
      report_rows[[length(report_rows) + 1]] <- data.frame(
        ...
        details = paste(
          "Rate:",
          round(results$mobility_report$mobility_rate[lowest_mobility_idx], 3)
        ),
        stringsAsFactors = FALSE
      )
    }
    ...
    if (
      length(max_inflow_idx) > 0 &&
        results$mobility_report$net_flow[max_inflow_idx] > 0
    ) {
      report_rows[[length(report_rows) + 1]] <- data.frame(
        ...
        details = paste(
          "Net flow:",
          round(results$mobility_report$net_flow[max_inflow_idx], 0)
        ),
        stringsAsFactors = FALSE
      )
    }
    ...
    if (
      length(max_outflow_idx) > 0 &&
        results$mobility_report$net_flow[max_outflow_idx] < 0
    ) {
      report_rows[[length(report_rows) + 1]] <- data.frame(
        category = "Flow Patterns",
        ...
        details = paste(
          "Net flow:",
          round(results$mobility_report$net_flow[max_outflow_idx], 0)
        ),
        stringsAsFactors = FALSE
      )
    }
...
#'
#' @keywords internal
determine_analysis_level <- function(
  moneca_results,
  enhanced_membership,
  level_selection
) {
  ...
  if (level_selection == "auto") {
    ...
    if (nrow(meaningful_levels) > 0) {
      ...
      level_nums <- as.numeric(sapply(
        strsplit(meaningful_levels$membership, "\\."),
        `[`,
        1
      ))
...
extract_segment_matrix <- function(moneca_results, enhanced_membership, level) {
  ...
  level_membership$level_num <- as.numeric(sapply(
    strsplit(level_membership$membership, "\\."),
    `[`,
    1
  ))
  ...
  if (nrow(level_data) > 0) {
    ...
    if (n_segments == 1 && nrow(unique_segments) == 1) {
      ...
      dimnames(mat) <- list(
        c(segment_names, "Total"),
        c(segment_names, "Total")
      )
    } else if (nrow(unique_segments) > 0) {
      # Multiple distinct segments with different names
      segment_names <- unique_segments$level_name[
        1:min(nrow(unique_segments), n_segments)
      ]
      ...
      if (length(segment_names) < n_segments) {
        ...
        additional_names <- paste0(
          "Segment_",
          original_names[(length(segment_names) + 1):n_segments]
        )
...
#'
#' @keywords internal
compute_segment_mobility_metrics <- function(
  segment_matrix,
  enhanced_membership
) {
  ...
  total_mobility <- sum(segment_matrix[1:n, 1:n]) -
    sum(diag(segment_matrix[1:n, 1:n]))

  # Compute metrics for each segment
  for (i in 1:n) {
    # Total flows
    mobility_metrics$total_outflow[i] <- sum(segment_matrix[i, 1:n]) -
      segment_matrix[i, i]
    mobility_metrics$total_inflow[i] <- sum(segment_matrix[1:n, i]) -
      segment_matrix[i, i]
    mobility_metrics$net_flow[i] <- mobility_metrics$total_inflow[i] -
      mobility_metrics$total_outflow[i]
    ...
    if (row_total > 0) {
      ...
      mobility_metrics$mobility_rate[i] <- 1 -
        mobility_metrics$immobility_rate[i]
    } else {
    ...
    # Directional mobility (assuming lower index = higher status)
    mobility_metrics$upward_mobility[i] <- if (i > 1) {
      sum(segment_matrix[i, 1:(i - 1)])
    } else {
      0
    }
    mobility_metrics$downward_mobility[i] <- if (i < n) {
      sum(segment_matrix[i, (i + 1):n])
    } else {
      0
    }
    ...
    total_segment_mobility <- mobility_metrics$total_outflow[i] +
      mobility_metrics$total_inflow[i]
    mobility_metrics$relative_mobility_share[i] <- if (total_mobility > 0) {
      total_segment_mobility / total_mobility
    } else {
      0
    }
  }
...
#'
#' @keywords internal
compute_segment_centrality_metrics <- function(
  segment_matrix,
  enhanced_membership
) {
  n <- nrow(segment_matrix) - 1 # Exclude totals row
  ...
  eigen_result <- tryCatch(
    igraph::eigen_centrality(g, directed = TRUE),
    error = function(e) list(vector = rep(0, n))
  )
  pagerank_result <- tryCatch(
    igraph::page_rank(g, directed = TRUE),
    error = function(e) list(vector = rep(0, n))
  )
  ...
  centrality_metrics <- centrality_metrics[
    order(centrality_metrics$strength_total, decreasing = TRUE),
  ]
...
#'
#' @keywords internal
generate_segment_plot <- function(
  segment_matrix,
  enhanced_membership,
  mobility_metrics,
  centrality_metrics,
  ...
  if (
    ...
    if (requireNamespace("ggplot2", quietly = TRUE)) {
      return(
        ggplot2::ggplot() +
          ggplot2::labs(
            title = "Cannot generate plot: insufficient data or missing packages"
          ) +
          ggplot2::theme_void()
      )
    } else {
  ...
  if (length(segment_names) != n_vertices) {
    warning(paste(
      "Mismatch: segment_names length",
      length(segment_names),
      "vs graph vertices",
      n_vertices
    ))
    ...
    if (length(segment_names) < n_vertices) {
      segment_names <- c(
        segment_names,
        paste("Node", (length(segment_names) + 1):n_vertices)
      )
    }
  }
  ...
  if (nrow(mobility_metrics) > 0) {
    ...
    mobility_subset <- mobility_metrics[
      mobility_metrics$segment %in% node_data$name,
    ]
  ...
  if (nrow(centrality_metrics) > 0) {
    ...
    centrality_subset <- centrality_metrics[
      centrality_metrics$segment %in% node_data$name,
    ]
  ...
  size_value <- switch(
    node_size_metric,
    "strength" = if ("strength_total" %in% names(node_data)) {
      node_data$strength_total
    } else {
      3
    },
    "mobility_rate" = if ("mobility_rate" %in% names(node_data)) {
      node_data$mobility_rate * 10
    } else {
      3
    },
  ...
  color_aes <- switch(
    ...
    "centrality" = if ("strength_total" %in% names(node_data)) {
      "strength_total"
    } else {
      NULL
    },
  ...
  if (!is.null(color_aes)) {
    p <- p +
      ggraph::geom_node_point(
        ggplot2::aes_string(color = color_aes),
        size = size_value
      )
  } else {
...
segment.membership.dataframe <- function(moneca_results, names = TRUE) {
  ...
  for (level in 1:n_levels) {
    ...
    if (level == 1) {
      ...
      if (length(unassigned_nodes) > 0) {
        ...
        previous_groups <- split(
          unassigned_nodes,
          previous_entity_assignment[unassigned_nodes]
        )
      ...
      for (entity_num in 1:entity_counter) {
        ...
        entity_assignment[entity_nodes] <- sprintf(
          paste0("%0", format_width, "d"),
          entity_num
        )
  ...
  id_full <- apply(positional_codes, 1, function(x) {
