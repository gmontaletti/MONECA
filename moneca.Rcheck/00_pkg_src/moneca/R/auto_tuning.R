...
# Internal helper: Simple parallel decision function
# Simplified version of the original should_use_parallel from smart_parallel_switching.R
should_use_parallel <- function(n_combinations, matrix_size, n_bootstrap = 50,
                                user_preference = "auto", verbose = FALSE) {

  # Convert logical preferences to strings
  ...
  if (is.na(n_cores) || n_cores < 2) {
    decision$reasoning <- "Insufficient cores for parallel processing"
    return(decision)
  }

  # Use parallel if workload is substantial
  workload_score <- n_combinations * n_bootstrap

  if (user_preference == "parallel" || (user_preference == "auto" && workload_score > 100)) {
    decision$use_parallel <- TRUE
    decision$n_cores <- max(1, min(n_cores - 1, 4))  # Leave 1 core free, cap at 4
    decision$reasoning <- "Parallel processing enabled"
  }

  if (verbose) {
    cat("Parallel decision:", ifelse(decision$use_parallel, "YES", "NO"),
        "(", decision$n_cores, "cores)\n")
  }
...
assess_clustering_stability_parallel <- function(mx,
  ...
  if (use_parallel) {
    ...
    # Create cluster and run parallel bootstrap using base parallel
    ...
    parallel::clusterExport(cl, c("mx", "sample_size", "n", "segment.levels",
                                 ...
                                 "process_bootstrap_sample", "bootstrap_seeds"),
                          envir = environment())
    ...
    # Run parallel bootstrap using parLapply
    bootstrap_signatures <- parallel::parLapply(cl, seq_len(n_bootstrap), function(i) {
      process_bootstrap_sample(i, bootstrap_seeds[i])
    })
