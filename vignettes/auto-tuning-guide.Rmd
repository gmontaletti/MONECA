...
test_data <- generate_mobility_data(
  ...
  noise_level = 0.15,
  class_names = paste0("Group_", 1:8)
)

# Test different manual parameters
manual_params <- c(2, 5, 10, 20)
manual_results <- list()
...
for(i in seq_along(manual_params)) {
  ...
  # Run clustering
  result <- moneca(test_data, segment.levels = 2, 
                   small.cell.reduction = param)
  
  # Store basic metrics
  manual_results[[i]] <- list(
    parameter = param,
    n_segments_level1 = length(unique(result$segment.list[[1]])),
    n_segments_level2 = length(unique(result$segment.list[[2]])),
    total_segments = length(unique(result$segment.list[[1]])) + 
                     length(unique(result$segment.list[[2]]))
  )
}
...
manual_df <- do.call(rbind, lapply(manual_results, function(x) {
  data.frame(
    ...
    Level1_Segments = x$n_segments_level1,
    Level2_Segments = x$n_segments_level2,
    Total_Segments = x$total_segments
  )
}))
...
cat("\nKey observations:\n")
cat("- Results vary dramatically with parameter choice\n")
...
stability_result <- moneca(
  ...
  segment.levels = 2,
  auto_tune = TRUE,
  auto_tune_method = "stability",
  n_trials = 15,  # More trials for better stability assessment
...
```{r performance_method, eval=TRUE}
...
for(w in performance_weights) {
  ...
  result <- moneca(
    ...
    segment.levels = 2,
    auto_tune = TRUE,
    auto_tune_method = "performance",
    performance_weight = w,
    seed = 456,
    verbose = TRUE
  )
...
medium_result <- moneca(test_data, segment.levels = 2,
                       ...
                       performance_weight = 0.3, seed = 456, verbose = FALSE)
...
cv_result <- moneca(
  ...
  segment.levels = 2,
  auto_tune = TRUE,
  auto_tune_method = "cross_validation",
  n_trials = 5,  # 5-fold CV
  seed = 456,
  verbose = TRUE
)
...
custom_result <- moneca(
  ...
  auto_tune = TRUE,
  auto_tune_method = "stability",
  candidate_values = custom_candidates,
  seed = 456,
  verbose = TRUE
)
...
```{r method_comparison, eval=TRUE, fig.width=16, fig.height=12}
...
comp_df <- do.call(rbind, lapply(comparison_results, function(x) {
  data.frame(
    Method = stringr::str_to_title(x$method),
    Parameter = x$parameter,
    Score = round(x$score, 4),
...
for(method in comparison_methods) {
  ...
  plots[[method]] <- plot_moneca_ggraph(
    result_data$result,
    title = paste0(stringr::str_to_title(method), " Method\n",
...
```{r performance_benchmark, eval=TRUE}
...
# For general use:
result_general <- moneca(data, auto_tune = TRUE, auto_tune_method = "stability")

# For maximum quality:
result_quality <- moneca(data, auto_tune = TRUE, auto_tune_method = "quality")

# For large datasets:
result_large <- moneca(data, auto_tune = TRUE, auto_tune_method = "performance", 
                       performance_weight = 0.4)

# For research/exploration:
result_research <- moneca(data, auto_tune = TRUE, auto_tune_method = "pareto")
...
|----------|-------------------|----------------|
| **General analysis** | `stability` | `n_trials = 10-20` |
| **Publication quality** | `quality` | Default settings |
| **Large datasets (>20×20)** | `performance` | `performance_weight = 0.3-0.5` |
| **Computational constraints** | `performance` | `performance_weight = 0.6-0.8` |
| **Research/exploration** | `pareto` | Default settings |
| **Small datasets** | `cross_validation` | `n_trials = 5-10` |
| **Reproducible results** | Any method | Always set `seed` |

### Performance Optimization Tips

```{r performance_tips, eval=TRUE}
# 1. Parallel processing for large datasets
cat("=== Performance Optimization Tips ===\n\n")

cat("1. Use parallel processing:\n")
cat('   moneca(data, auto_tune = TRUE, parallel_cores = 4)\n\n')

cat("2. Limit candidate values for faster search:\n")
cat('   moneca(data, auto_tune = TRUE, max_candidates = 10)\n\n')

cat("3. Use performance method for speed:\n")
cat('   moneca(data, auto_tune = TRUE, auto_tune_method = "performance",\n')
cat('          performance_weight = 0.5)\n\n')

cat("4. Reduce trials for faster results:\n")
cat('   moneca(data, auto_tune = TRUE, n_trials = 5)\n\n')

cat("5. Enable caching for repeated analyses:\n")
cat('   moneca(data, auto_tune = TRUE, use_cache = TRUE)\n\n')
```

### Reproducibility Guidelines

```{r reproducibility, eval=TRUE}
...
fast_result <- moneca(data, auto_tune = TRUE, auto_tune_method = "performance",
                      performance_weight = 0.6)

# Solution 2: Reduce candidates
fast_result <- moneca(data, auto_tune = TRUE, max_candidates = 8)

# Solution 3: Fewer trials
fast_result <- moneca(data, auto_tune = TRUE, n_trials = 5)

# Solution 4: Parallel processing
fast_result <- moneca(data, auto_tune = TRUE, parallel_cores = 4)
```
...
# Solution 1: Set seed for reproducibility
stable_result <- moneca(data, auto_tune = TRUE, seed = 123)

# Solution 2: Increase trials for better stability
stable_result <- moneca(data, auto_tune = TRUE, n_trials = 20)
...
better_result <- moneca(data, auto_tune = TRUE, auto_tune_method = "quality")

# Solution 2: Expand search space
better_result <- moneca(data, auto_tune = TRUE, 
                       candidate_values = c(1, 2, 3, 5, 8, 12, 20, 30, 50))

# Solution 3: Try multiple methods and compare
methods <- c("stability", "quality", "performance")
results <- lapply(methods, function(m) {
  moneca(data, auto_tune = TRUE, auto_tune_method = m, seed = 123)
})
```
...
memory_result <- moneca(data, auto_tune = TRUE, use_cache = FALSE)

# Solution 2: Use performance method (more memory efficient)
memory_result <- moneca(data, auto_tune = TRUE, auto_tune_method = "performance")

# Solution 3: Reduce matrix size or segment levels
...
batch_results <- lapply(datasets, function(data) {
  moneca(data, auto_tune = TRUE, auto_tune_method = "stability", seed = 123)
})
