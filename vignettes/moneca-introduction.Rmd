...
```{r advanced_auto_tuning, eval=TRUE}
...
| `auto_tune_method` | Tuning method | `"stability"` | See methods below |
| `n_trials` | Number of trials/folds | `10` | Higher = more robust |
| `performance_weight` | Speed vs quality balance | `0.3` | 0 = quality only, 1 = speed only |
| `candidate_values` | Custom parameter range | `NULL` | Auto-generated if `NULL` |
| `seed` | Reproducibility seed | `NULL` | Set for consistent results |
...
5. **Parallel processing available**: Use `parallel_cores` parameter for speedup
